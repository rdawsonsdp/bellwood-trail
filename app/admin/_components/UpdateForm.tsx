...
export function UpdateForm({ update, version, today }: { update?: Update; version: string; today: string }) {
  ...
  return (
    <div className="space-y-6">
      <Link href="/admin#updates" className="text-small font-semibold text-warm-gray hover:text-ink">← All updates</Link>
      ...
      <form onSubmit={keepOnError(action)} className="space-y-5 rounded-2xl border border-line bg-paper p-5 sm:p-6">
        ...
        <label className="block">
          ...
          <span className="mt-1 block text-xs text-warm-gray">A sentence or two. Shown in full on the card.</span>
        </label>
        <label className="block">
          ...
          <span className="mt-1 block text-xs text-warm-gray">Adds a “Read more” link to the card.</span>
        </label>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/admin#updates" className="text-small font-semibold text-warm-gray hover:text-ink">Cancel</Link>
        </div>
      </form>
    </div>
  );
