...
export async function SiteHealth({ site, address, live }: { site: string; address: string; live: boolean }) {
  ...
  if (!s.ok) return <span className="font-semibold text-blue">⚠ {s.error}</span>;
  if (!live) return <span className="text-muted">Site up · card uses stored details</span>;
  const got = [s.schedule && "hours", s.phone && "phone", s.address && "address"].filter(Boolean);
  return got.length
    ? <span className="text-open">● Live: {got.join(", ")}</span>
    : <span className="font-semibold text-gold-ink">Site up, but it publishes no hours, phone or address — card uses stored details</span>;
}
