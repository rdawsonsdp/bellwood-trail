      ...
      <header className="sticky top-0 z-40 border-b border-line bg-paper/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-[1100px] items-center justify-between gap-4 px-5">
          <Link href="/admin" className="flex items-center gap-3">
            <Image src="/images/brand/bellwood-logo.png" alt="" aria-hidden width={400} height={170} className="h-9 w-auto" />
          ...
          <nav className="flex shrink-0 items-center gap-3 whitespace-nowrap text-xs font-semibold sm:gap-5 sm:text-small">
            <a href="/" target="_blank" rel="noopener noreferrer" className="text-muted hover:text-ink">View site ↗</a>
            <form action={logout}><button className="text-blue hover:underline">Log out</button></form>
          </nav>
        </div>
      </header>
      {note && <p className="border-b border-sand/40 bg-sand/15 px-5 py-2 text-center text-small text-ink">{note}</p>}
