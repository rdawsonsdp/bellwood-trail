...
const NAV = [{ label: "Explore kitchens", href: "#path" }, { label: "The neighborhood", href: "#neighborhoods" }, { label: "Latest", href: "#updates" }];
export function Header() {
  ...
  return <header className="discovery-header" onKeyDown={e => { if (e.key === "Escape") { setOpen(false); menu.current?.focus(); } }}>
    ...
    {open && <nav id="mobile-navigation" className="mobile-navigation" aria-label="Mobile navigation">{NAV.map(n => <a key={n.href} href={n.href} onClick={e => { setOpen(false); if (n.href === "#path") { e.preventDefault(); updateFilters(EMPTY_FILTERS, true); } }}>{n.label}</a>)}<a href={SITE.orgUrl} target="_blank" rel="noopener noreferrer">About Greater Chatham Initiative</a></nav>}
  </header>;
}
